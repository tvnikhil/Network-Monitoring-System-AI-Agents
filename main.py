...
path_to_file = "capture.pcap"
initial_duration = 18
initial_cycle_interval = 2
deps = MyDeps(pathToFile=path_to_file, duration=initial_duration, cycle_interval=initial_cycle_interval)
...
while True:
    ...
    if should_capture("last15.json"):

        prompt = f"Tune parameters based on current network conditions. Here is the current average latency: {deps.avg_latency} and current packet loss: {deps.avg_loss}"
        print(prompt)
